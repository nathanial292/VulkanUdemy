...
int VulkanRenderer::init(GLFWwindow* newWindow)
{
	...
	try {
		...
		mvp.projection = glm::perspective(glm::radians(70.0f), (float)swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 100.0f);
		mvp.view = glm::lookAt(glm::vec3(0.0f, 1.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		mvp.model = glm::mat4(1.0f);

		mvp.projection[1][1] *= -1; // Invert the y axis for vulkan (GLM was made for opengl which uses +y as up)
...
void VulkanRenderer::createDescriptorSetLayout()
{
	...
	mvpLayoutBinding.pImmutableSamplers = nullptr; // Not using any textures atm (For texture: can make sampler data immutable by specifying in layout

	// Create Descriptor Set Layout with given bindings
	VkDescriptorSetLayoutCreateInfo createInfo = {};
	createInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
	createInfo.bindingCount = 1; // Number of binding infos
	createInfo.pBindings = &mvpLayoutBinding; // Pointer to binding info
...
{
	// Buffer size will be of size of all three variables (will offset to access)
...
{
	// Type of descriptors + how many descriptors (not descriptor sets) We only have only one descriptor in our shader (MVP matrix)
...
{
	descriptorSets.resize(uniformBuffer.size()); // Resize descriptor set list so one for every buffer
...
#
	...
	for (size_t i = 0; i < descriptorSets.size(); i++) {
		...
		mvpBufferInfo.offset = 0; // Position where data starts
